      ...
      <Section title="The team they'll manage">
        {reports.map((r, i) => (
          <div key={r.id} className="rounded-lg p-3 mb-3" style={{ background: "var(--surface-2, rgba(255,255,255,0.05))", border: "1px solid var(--border, rgba(255,255,255,0.07))" }}>
            ...
            <input value={r.personality_traits.join(", ")} onChange={(e) => setReport(r.id, { personality_traits: e.target.value.split(",").map((s) => s.replace(/^ +/, "")) })} onBlur={(e) => setReport(r.id, { personality_traits: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} className="w-full rounded-lg p-2 text-[13px] mb-2" style={field} placeholder="Personality traits, comma-separated (e.g. blunt, perfectionist, resents the reorg)" />
